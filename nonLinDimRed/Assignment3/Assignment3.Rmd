...
find.nearest <- function(new.pos){
  # Compute distance to points and select nearest index
  return(which.min(colSums((t(conf) - new.pos)^2)))
}


chosen.points.localMDS <- rbind(conf[find.nearest(c(-100,0)),], 
                                conf[find.nearest(c(-40,0)), ], 
                                conf[find.nearest(c(0,0)), ], 
                                conf[find.nearest(c(20,0)), ], 
                                conf[find.nearest(c(60,0)), ], 
                                conf[find.nearest(c(40,-25)), ], 
                                conf[find.nearest(c(40,0)),], 
                                conf[find.nearest(c(40,20)), ],
                                conf[find.nearest(c(40,50)), ])

fnci <- function(p){
  # Use to find indices of the chosen points. 
  which(conf == p, arr.ind = T)
}
chosen.indices <- apply(chosen.points.localMDS, FUN = fnci, 1)[1,]
chosen.points.localMDS <- data.matrix(chosen.points.localMDS)
...
plot3d(scores[,1], scores[,2], scores[,3], pch=19,cex=.5, col = 2, 
       xlim = range(scores[,1]), ylim = range(scores[,2]), zlim = range(scores[,3])) 
# Not sure why it does not plot anything.
```
...
```{r}
chosen.points.ISO <- rbind(ismp.points[find.nearest(c(-80,0)),], 
                                ismp.points[find.nearest(c(-40,0)), ], 
                                ismp.points[find.nearest(c(0,0)), ], 
                                ismp.points[find.nearest(c(20,0)), ], 
                                ismp.points[find.nearest(c(60,0)), ], 
                                ismp.points[find.nearest(c(40,-25)), ], 
                                ismp.points[find.nearest(c(40,0)),], 
                                ismp.points[find.nearest(c(40,20)), ],
                                ismp.points[find.nearest(c(40,50)), ])

fnci2 <- function(p){
  # Use to find indices of the chosen points. 
  which(ismp.points == p, arr.ind = T)
}
chosen.indices.ISO <- apply(chosen.points.ISO, FUN = fnci2, 1)[1,]
chosen.points.ISO <- data.matrix(chosen.points.ISO)
...
The same conclusions as for local MDS can be drawn from these plots. When looking more closely at the points selected, several of them have the same index in the training data for both methods, which explains why several of the images are the same. Thus, the results are very similar. 
