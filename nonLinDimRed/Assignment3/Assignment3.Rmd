...
###  (OPTIONAL) Relate the results from Local MDS with those obtained by the first 3 principal components. In particular, could you represent in any way the results obtained by Local MDS in the 3-dimensional scatterplot of (PC1, PC2, PC3)?


Below the dimensions found using Local MDS are plotted against the first three
principal components. We observe that the first dimension of the Local MDS 
posetively correlates with the first PC. Similarly the second dimension of the 
Local MDS correlates negatively with the second PC. Third PC does not show as 
clear of a correlations, but it absolutely show a pattern.
To create the plots we fit a GAM using splines for each principal component against
the coordinates in the Local MDS dimensions.
Finally we plot all three together in the PC1,PC2,PC3 plane.
The plot seems reasonable, but is hard to read and interpret.
...
As before, the zeros corresponding to the red points are plotted. The grid of zeros now directly corresponds to the, somewhat skewed, grid of red points above. When inspecting, we can see that the traversion from west to east seems to describe the width of the zeros while the traversion from south to north could be describing the height of the zero.


As before we first plot the two ISOMAP dimensions found against the first
three principal components separately. It looks like the first ISOMAP 
dimension positively correlates with the first PC (very similar to local MDS).
The second dimension negatively correlates with the third PC, while the 
correlation with the second PC is harder to interpret. We observe that the plots
are very similar to the equivalent plots using Local MDS, with the exception
that the second dimension now correlates with PC3 and not PC2. 
This behaviour is supported by the plots of zeros, as we see similar behaviour
as in local MDS moving along the first dimension, but different when moving 
along the second dimension.
Finally the fitted points are plotted in the three dimensional PC space.

```{r}
zip.PC <- princomp(zip.zeros[,-1])
scores <- zip.PC$scores[,1:3]

smooth.pc1 <- gam(scores[,1] ~ s(ismp.points[,1],ismp.points[,2]))
smooth.pc2 <- gam(scores[,2] ~ s(ismp.points[,1],ismp.points[,2]))
smooth.pc3 <- gam(scores[,3] ~ s(ismp.points[,1],ismp.points[,2]))

s.pc1 <- smooth.pc1$fitted.values
s.pc2 <- smooth.pc2$fitted.values
s.pc3 <- smooth.pc3$fitted.values

points3D(ismp.points[,1], ismp.points[,2], s.pc1,
         xlab = "Dim1", ylab = "Dim2", zlab ="PC1")
points3D(ismp.points[,1], ismp.points[,2], s.pc2,
         xlab = "Dim1", ylab = "Dim2", zlab ="PC2")
points3D(ismp.points[,1], ismp.points[,2], s.pc3,
         xlab = "Dim1", ylab = "Dim2", zlab ="PC3")
...
As earlier, the first five images show the traversion from west to east in the first dimension, while the last four images show the traversion from south to north in the second dimension. 
The first dimension seems to describe the width of the zero, but unlike for 
local MDS and ISOMAP the wide zeros are on the left, and narrow on the right.
The second dimension is harder to interpret, it can seem like low values give nice
round zeros, while higher values give skewed and "flawed" zeros.

Below we fit the dimensions against the first three principal components as before.
Firstly we observe that the first dimension correlates negatively with the first principal component. 
From before we know that PC1 has to do with the width of the number, so this matches our expectation. 
The correlations are more complicated for both PC2 and PC3, and there is no clear
correlation in one dimension. 
This can explain our difficulty in interpreting the numbers moving in the second
dimension. 
Maybe it is a combination between height of the number and line width.

```{r}
zip.PC <- princomp(zip.zeros[,-1])
scores <- zip.PC$scores[,1:3]

smooth.pc1 <- gam(scores[,1] ~ s(tsne.points[,1],tsne.points[,2]))
smooth.pc2 <- gam(scores[,2] ~ s(tsne.points[,1],tsne.points[,2]))
smooth.pc3 <- gam(scores[,3] ~ s(tsne.points[,1],tsne.points[,2]))

s.pc1 <- smooth.pc1$fitted.values
s.pc2 <- smooth.pc2$fitted.values
s.pc3 <- smooth.pc3$fitted.values

points3D(tsne.points[,1], tsne.points[,2], s.pc1,
         xlab = "Dim1", ylab = "Dim2", zlab ="PC1")
points3D(tsne.points[,1], tsne.points[,2], s.pc2,
         xlab = "Dim1", ylab = "Dim2", zlab ="PC2")
points3D(tsne.points[,1], tsne.points[,2], s.pc3,
         xlab = "Dim1", ylab = "Dim2", zlab ="PC3")
