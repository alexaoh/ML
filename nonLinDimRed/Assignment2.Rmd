...
It seems reasonably close to the generator curve in red, and captures the bent "S"-shape.
...
Now, with df = 50, we get the significantly lower error 2.956.
...
However, fitting a curve with 50 degrees of freedom from only 75 data points immediately sounds prone to overfitting and in general bad practice.
We plot the principal curve with df = 50 to see why this can happen.
...
In the extreme case where df goes to infinity, the principal curve can "fill" the space, and every random point in the space will be infinitely  close to its projection. 