...
# Snittbildene virker sannsynlige basert på plottene nedenfor for hver cluster!
# Dette kan fjernes til slutt, var bare for å "sjekke" om snittene over gir mening. 
par(mfrow=c(3,4))
dat <- zip.zeros %>% dplyr::filter(cluster == 4)
apply(dat[sample(1:nrow(dat), 12), -c(1,ncol(dat))],1,plot.zip, use.first = T)
```

### Principal Components

The principal components for this dataset are calculated. The scatterplot of the scores in the two first components are plotted, using different colors for points in different clusters. 
...
sm.density(df.clust3[, c(1,2)], display="slice",props=c(75), add = T)
sm.density(df.clust4[, c(1,2)], display="slice",props=c(75), add = T)
# princ.curve <- principal_curve(zip.PC$scores[,1:2])
...
# lines(princ.curve, col = 1, lty = 2, lwd = 2) # Vet ikke om dette er korrekt, bare prøvde. 
# Dette stemmer bedre med eksempler fra forelesning
...
# lines(princ.curve, col = 1, lty = 2, lwd = 2) # Vet ikke om dette er korrekt, bare prøvde. 
lines(proj.prcv.to.PC[princ.curve$ord,1:2],col=1, lty = 2, lwd=2)
# Her er det nok noe galt ja! ;))

# Skjønner ikke hvordan vi skal kunne plotte disse over PC-plottet.
# Plottene nedfor gir mening ift rekkefølgen på clusterne langs PRCV
sm.density(df2.clust1[, 1], add = F)
sm.density(df2.clust2[, 1], add = F)
sm.density(df2.clust3[, 1], add = F)
sm.density(df2.clust4[, 1], add = F)

# sm.density(df2.clust2[, 1], add = T)
# sm.density(df2.clust3[, 1], add = T)
# sm.density(df2.clust4[, 1], add = T)
```
