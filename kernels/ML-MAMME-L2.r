...
# The SVM is very costly for medium to large datasets.
# A useful alternative (specially when the number of variables is not small) is to use the {LiblineaR} package
...
# This code is heavily based on J.P. Vert's excelllent teaching material
...
## b) Next, this is how we would do normally do it (since this is a built-in kernel)
...
## The resulting dataset is a list of pairs (Topic, News). We will use three topics for 
# analysis: Crude Oil, Coffee and Grain-related news
...
# To deal with textual data we need to use a string kernel. Several such kernels 
#are implemented in the "stringdot" method of the kernlab package. 
#We shall use the simplest one: the n-spectrum kernel. The feature map represents 
#the string as a multiset of its substrings of length n
...
legend("topright", legend=c("crude oil", "coffee","grain"),    
...
# of course it is still symetric, but
isSymmetric.matrix(K)
...
eigen(K.n, only.values = TRUE)$values # K is pd

sort(rowSums(S))
sort(rowSums(K.n))

# A drawback (?) is that the trace is smaller:
library(psych)
tr(K) > tr(K.n)
...
plot(g, vertex.label.dist=1,vertex.size=4)
